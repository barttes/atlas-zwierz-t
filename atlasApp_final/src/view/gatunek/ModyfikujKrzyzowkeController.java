 ...
 * @author Dell
 */
public class ModyfikujKrzyzowkeController implements Initializable {
     ...
     * @param gatunekTemp Krzyżówka przekazana z głównego okna.
     */
     ...
     * @param gatunekTemp Krzyżówka przekazana do metody
     */
    public void pokazDetaleGatunku(GatKrzyz gatunekTemp) {
        ...
        listaGatunkow.setText(gatunekTemp.listaGatunkowDoWyswietlenia(gatunekTemp.getListaGatunkow()));
    }
     ...
     */
     public void zmienOknoZapisz(ActionEvent event) throws IOException {
        if (!nazwaGatunku.getText().equals("")) {
            ...
            List<String> listaTempOld = gatunekTemp.getListaGatunkow();
            try {
                atlas.getListaTypow().get(idTypu).getListaRodzin().get(idRodziny).getListaGatKrzyz().get(idGatunku).setListaGatunkow(Arrays.asList(listaGatunkow.getText().split(",")));
            } catch (Exception e) {
                atlas.getListaTypow().get(idTypu).getListaRodzin().get(idRodziny).getListaGatKrzyz().get(idGatunku).setListaGatunkow(listaTempOld);
            ...
            ObslugaAlertow.niepelneDane("Nazwa krzyżówki musi być podana ..");
        }
    }
}
